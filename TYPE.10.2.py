...
class HackerButton(tk.Button):
    def __init__(self, master, **kwargs):
        ...
        kwargs.setdefault('activebackground', "white")
        kwargs.setdefault('activeforeground', "black")
        kwargs.setdefault('bd', 2)
        kwargs.setdefault('relief', "flat")
        super().__init__(master, **kwargs)
        self.bind("<Enter>", lambda e: self.config(bg="white", fg="#00E5FF"))
...
# --- Training Logic --- ___________________________________________________________________________________________________________________________

def start_weakspot_training(): # Find chars with highest error rate
    load_user_stats()
...
def load_user_stats():
    ...
    for char, correct, wrong in cursor.fetchall():
        user_stats[char] = {
            "correct": correct,
            "wrong": wrong
        }
...
def get_char_stat(c):
    return user_stats.get(c, {"correct": 0, "wrong": 0})
...
def refresh_heatmap():
        ...
        for row_idx, row in enumerate(rows):
            ...
            for col_idx, char in enumerate(row):
                x = start_x + col_idx * key_spacing + (row_idx * 30)
...
def show_mode_info():
    messagebox.showinfo(
        "Training Modes",
        "ACCURACY MODE:\n"
        "- Trains worst accuracy (wrong / attempts)\n"
        "- Focuses on weakest precision keys\n\n"

        "WEIGHTED MODE:\n"
        "- Combines accuracy + total mistakes\n"
        "- Focuses on keys you struggle with most overall\n\n"

        "Tip:\n"
        "- Use WEIGHTED for faster improvement\n"
        "- Use ACCURACY for fine-tuning"
    )
...
def toggle_training_mode():
    ...
    mode_label.config(text=f"MODE: {training_mode.upper()}")
...
for widget in main_menu_container.winfo_children():
    if isinstance(widget, tk.Label) and "MODE:" in widget.cget("text"):
        widget.destroy()
    if isinstance(widget, tk.Button) and "TOGGLE TRAINING MODE" in widget.cget("text"):
        widget.destroy()
...
mode_row = tk.Frame(l_frame_container, bg="black")
...
# Toggle button that shows current mode
def update_mode_button_text():
    toggle_btn.config(text=f"[ MODE: {training_mode.upper()} ]")

def toggle_training_mode():
    global training_mode
    training_mode = "weighted" if training_mode == "accuracy" else "accuracy"
    update_mode_button_text()

toggle_btn = HackerButton(
    mode_row,
    text=f"[ MODE: {training_mode.upper()} ]",
    command=toggle_training_mode,
    width=20
)
